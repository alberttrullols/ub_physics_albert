...
c MCTOT nombre de passes MC
c MCINI nombre de passes que saltarem per als promitjos
c MCD cada MCD passes promitgem
      MCINI=1000
      MCD=10
      N=L*L
...
c INICIALITZACIÓ VARIABLES PROMIG
      ...
      OPEN (UNIT=12, FILE = TRIM (NOM)//"_H.out")
...
c ALGORITME METROPOLIS
      ...
      do i = 1, MCTOT
       do j = 1, N
        ...
        a=ceiling(x*(L-1)+1.d0)
        b=ceiling(y*(L-1)+1.d0) 
...
c definim la function energia
     ...
     F         S(i,j)*S(i,PBC(j-1))-S(i,j)*S(PBC(i-1),j)
       end do
      end do 
